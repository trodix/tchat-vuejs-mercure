...
Grab a binary from the release page and run:
    ...
    JWT_KEY='myJWTKey' ADDR=':3000' DEMO=1 ALLOW_ANONYMOUS=1 PUBLISH_ALLOWED_ORIGINS='http://192.168.0.38:3000' ./mercure

The server is now available on `http://192.168.0.38:3000`, with the demo mode enabled. Because `ALLOW_ANONYMOUS` is set to `1`, anonymous subscribers are allowed.
